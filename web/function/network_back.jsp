...
*/
StringBuffer query=new StringBuffer("select  ?name1  ?name2 ?topic1 ?topic2   ?postcode where {  ?a1 foaf:knows ?b1 . "
+"  ?a1 rdfs:label ?name1.  ?a1 a foaf:Person. "
+"  ?b1 a foaf:Person.  "
        + "optional {?a1 st:Topic ?topic1.}"
+" optional { ?b1 st:Topic ?topic2.}"
+"  ?b1 rdfs:label ?name2. "
+ "  ?a1 foaf:mbox ?postcode.   ");
...
//     	HashMap<String, Integer> name = new HashMap<String, Integer>();
		...
		int num=0; 
                while (res.hasNext()) {
					...
					Binding tempTop=bs.getBinding("topic1");
//                                        out.println(name1);
					if (tempTop!=null) {
   					String topic1 =bs.getBinding("topic1").getValue().stringValue(); 
				        topic1=topic1.toUpperCase();
					
					if(topic1.indexOf("UQ")>=0&&name1.indexOf("UQ_News")==-1) groupMap.put(name1,new Integer(2));
					}
					if(name1.indexOf("RCCUQ")>=0) groupMap.put(name1,new Integer(4));
					if(name1.indexOf("UQ_News")>=0) groupMap.put(name1,new Integer(4));
					if(name1.indexOf("oneinbillion"	)>=0) groupMap.put(name1,new Integer(3));
					if(name1.indexOf("Antonpjm"	)>=0) groupMap.put(name1,new Integer(3));
						
					tempTop=bs.getBinding("topic2");
					if (tempTop!=null){
   					String topic2 =bs.getBinding("topic2").getValue().stringValue(); 
				        topic2=topic2.toUpperCase();
	//				out.println(topic2);
//					if(topic2.indexOf("UQ")>=0) groupMap.put(name2,new Integer(2));
					if(topic2.indexOf("UQ")>=0&&name2.indexOf("UQ_News")==-1) groupMap.put(name2,new Integer(2));
}
					if(name2.indexOf("oneinbillion"	)>=0) groupMap.put(name2,new Integer(3));
					if(name2.indexOf("Antonpjm"	)>=0) groupMap.put(name2,new Integer(3));
  // 					if(name.get(name1)==null){
    //                                            name.put(name1,count++);
	//				}	
	//				if(name.get(name2)==null){
          //                                  name.put(name2,count++);
	//				}
